...
export const SAFE_ZONE_RADIUS_M = 40;
...
export default function PetMap({
  ...
  const distFromHome = haversineM(
    fenceOrigin.latitude,
    fenceOrigin.longitude,
    center.latitude,
    center.longitude,
  );
  ...
  return (
    <div className="pet-map-wrap" style={{ height }}>
      ...
      >
        ...
        {showZones && <Marker position={toLL(fenceOrigin)} icon={homeIcon} />}
      </MapContainer>

      {showZones && (
        <div
          className={`pm-fence-badge ${insideSafeZone ? "pm-fence-in" : "pm-fence-out"}`}
        ...
        >
          {insideSafeZone ? (
            ...
            <>
              ...
              <span>Outside safe zone · {Math.round(distFromHome)} m from home</span>
            </>
          )}
        </div>
      )}
      ...
      <div className="pm-legend">
        {showPath && positions.length > 1 && (
          <span className="pm-legend-item">
            <span className="pm-ld pm-ld-start" />
            Start
          </span>
        )}
        <span className="pm-legend-item">
          <span className="pm-ld pm-ld-current" />
          Pet
        </span>
        {showZones && (
          <span className="pm-legend-item">
            <span className="pm-ld pm-ld-safe" />
            20 m safe zone
          </span>
        )}
      </div>
